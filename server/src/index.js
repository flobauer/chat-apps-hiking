...
async function loadAllTours() {
    ...
    return parsed.data.items.reduce((result, item) => {
...
async function loadTourDetail(summary) {
...
function formatTourForModel(tour) {
...
function formatMapOpenedMessage(query) {
    if (!query) {
...
const server = new McpServer({
    ...
    version: "0.0.1",
}, { capabilities: {} }).registerWidget("hiking-tour-map", {
    description: "Interactive hiking tour map of Austria with detailed tour information and photos.",
    _meta: {
        ui: {
            csp: {
                connectDomains: [
    ...
    description: "Use this tool to explore hiking tours in Austria. Displays an interactive map with detailed information about hiking tours including route, elevation, and beautiful photos. Always use it when users ask about hiking tours, trails, or want to explore outdoor activities.",
    inputSchema: {
        name: z
            .string()
            .optional()
            .describe("Optional hiking tour or area query, such as a route title or place name like 'Hinterstoder', 'Stoder', or 'Dachstein'. Leave empty to open the general hiking map."),
    },
    annotations: {
        readOnlyHint: true,
        openWorldHint: true,
    ...
    try {
        ...
        const tour = await getTourByName(name);
        return {
            _meta: {
                slug: tour.slug,
            ...
            content: [
                {
                    type: "text",
                    text: `${formatMapOpenedMessage(name)}\n\n${formatTourForModel(tour)}`,
                },
            ],
            isError: false,
        };
